...
class DisplayMyAddress extends Model {
    ...
    public static function myAddresssSummaryUser($userId, $type = FALSE) {
        $products = [];
        $dataAddress = \common\models\costfit\Address::find()->where("userId ='" . $userId . "' and isDefault =1 and type =" . $type)->orderBy('addressId DESC')->one();
        //echo 'countryName : ' . $dataAddress->countries->countryName;
        $products['myAddresss'] = [
            'addressId' => $dataAddress['addressId'],
            'userId' => $dataAddress['userId'],
