...
//print_r($model);
//echo 'DiscountProduct : ' . $DiscountProduct;
?>
    ...
    <div class="product-box">
        ...
        <div class="product-img text-center">
            <a href="<?= Url::to(Yii::$app->homeUrl . 'product/' . $model->encodeParams(['productId' => $model->productId])) ?>" class="fc-black">
                ...
                <!--<img class="media-object fullwidth img-responsive" src="https://www.cozxy.com/images/ProductImageSuppliers/thumbnail1/vOzzkm_AyOohuC3odOJIu-5DxjF3oKec.jpg">-->
            </a>
            <?php
            if ($UserAgent != 'mobile') {
                ?>
        ...
        <div class="product-txt">
            ...
            <p class="name">
                <a href="<?= Url::to(Yii::$app->homeUrl . 'product/' . $model->encodeParams(['productId' => isset($model->product->productId) ? $model->product->productId : $model->productId])) ?>" class="size14 b">
                    <?= isset($model->product) ? (strlen(strtoupper($model->product->title)) <= 40) ? strtoupper($model->product->title) : substr(strtoupper($model->product->title), 0, 40) : '' ?>
                </a>
            </p>
            <?php
            if ($model->price > 0) {
                if (isset($hotDeal)) {
                    ?>
                    <p class="price" >
                        <span class="size14 fc-red"><?= number_format($model->price) . ' THB' ?> </span>
