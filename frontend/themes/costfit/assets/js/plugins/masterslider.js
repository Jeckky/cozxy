...
(function () {
    ...
    p.start = function () {
        ...
        this.onTransComplete = function (event) {
            ...
            that.fresh = false;
            if (that.chained_tween)
                that.chained_tween.start();
            if (that.oc_fb)
        ...
        this.start_to = setTimeout(function () {
            ...
            if (that.delay > 0)
                that.$element.css(window._jcsspfx + 'TransitionDelay', that.delay + 'ms');
            else
                that.$element.css(window._jcsspfx + 'TransitionDelay', '');

            if (that.ease != 'linear')
                that.$element.css(window._jcsspfx + 'TransitionTimingFunction', that.ease);

            if (that.to_cb)
                that.to_cb.call(that.to_cb_target);
...
(function ($) {
    ...
    p.__playVideo = function () {
        if (this.img)
            CTween.fadeOut(this.img, 500, false);
        CTween.fadeOut(this.video_btn, 500, false);
        this.video_frame.attr('src', 'about:blank').css('display', 'block');
        if (this.video_url.indexOf('?') == -1)
            this.video_url += '?';
...
(function ($) {
    ...
    p._updateClassName = function (name) {
        if (this._lastClass)
            this.tt.removeClass(this._lastClass);
...
(function ($) {
    ...
    p.setSize = function (width, height, hard) {
        ...
        if (hard && this.selected)
            this.initLayers(hard);
        if (this.selected)
            this.locateLayers();
    ...
    p.__closeVideo = function () {
        ...
        CTween.animate(this.vcbtn, 500, {opacity: 0}, {complete: function () {
    ...
    p.create = function () {
        ...
        if (this.link) {
            this.$element.css('cursor', 'pointer')
                    .click(function () {
                        if (!that.linkdis)
                            window.open(that.link, that.link_targ || '_self');
                    });
        }
    ...
    p.unselect = function () {
        ...
        if (this.hasLayers) {
            var that = this;
            that.lht = CTween.animate(this.$layers, 500, {opacity: 0}, {complete: function () {
                    that.resetLayers();
                }});
...
(function ($) {
    ...
    p.setupView = function () {
        ...
        if (viewClass._3dreq && (!window._css3d || $.browser.msie))
    ...
    p.setup = function () {
        ...
        if (this.so.wheel) {
            ...
            if ($.browser.mozilla)
                this.slider.$element[0].addEventListener('DOMMouseScroll', this.wheellistener);
            else
    ...
    p._destroy = function () {
        ...
        if (this.so.wheel) {
            if ($.browser.mozilla)
                this.slider.$element[0].removeEventListener('DOMMouseScroll', this.wheellistener);
            else
...
(function ($) {
    ...
    p.__setupSlides = function () {
        ...
        this.$element.children('.ms-slide').each(function (index) {
            ...
            if (slide_video.length > 0)
                new_slide.setBGVideo(slide_video);
    ...
    p.__createSlideLayers = function (slide, layers) {
        ...
        layers.each(function (index, domEle) {
            ...
            if ($layer_element.data('effect') !== undefined)
                eff_parameters.name = $layer_element.data('effect');
            if ($layer_element.data('ease') !== undefined)
                eff_parameters.ease = $layer_element.data('ease');
            if ($layer_element.data('duration') !== undefined)
                eff_parameters.duration = $layer_element.data('duration');
            if ($layer_element.data('delay') !== undefined)
                eff_parameters.delay = $layer_element.data('delay');

            if ($layer_element.data('hide-effect'))
                end_eff_parameters.name = $layer_element.data('hide-effect');
            if ($layer_element.data('hide-ease'))
                end_eff_parameters.ease = $layer_element.data('hide-ease');
            if ($layer_element.data('hide-duration') !== undefined)
                end_eff_parameters.duration = $layer_element.data('hide-duration');
            if ($layer_element.data('hide-time') !== undefined)
                end_eff_parameters.time = $layer_element.data('hide-time');

            if ($layer_element.data('resize') !== undefined)
                layer.resizable = $layer_element.data('resize');
            if ($layer_element.data('swipe') !== undefined)
                layer.swipe = $layer_element.data('swipe');

            if ($layer_element.data('widthlimit') !== undefined)
                layer.minWidth = $layer_element.data('widthlimit');
    ...
    p.setHeight = function (value) {
        if (this.options.smoothHeight) {
            if (this.htween) {
                if (this.htween.reset)
                    this.htween.reset();
                else
                    this.htween.stop(true);
            }
    ...
    p.setup = function (id, options) {
        ...
        if (this.$element.length === 0) {
            if (console)
                console.log('Master Slider Error: #' + id + ' not found.');
            return;
        }
...
(function ($) {
    ...
    p.__snapUpdate = function (controller, snap, change) {
        ...
        if (this.loop) {
            ...
            if (target_index >= this.slidesCount)
                target_index = target_index - this.slidesCount;
            if (target_index < 0)
                target_index = this.slidesCount + target_index;

            this.index = target_index;
        } else {
            if (snap < 0 || snap >= this.slidesCount)
                return
            this.index = snap;
        }
        ...
        if (new_slide === this.currentSlide)
            return;
    ...
    p.__locateSlides = function (move, start) {
        ...
        if (move !== false)
            this.controller.changeTo(this.slideList[this.index].position, false, null, null, false);
    ...
    p.gotoSlide = function (index, fast) {
        ...
        if (target_slide === this.currentSlide)
            return;
    ...
    p.setSize = function (width, height, hard) {
        ...
        if (this.__created) {
            ...
            if (!this.loop)
                this.controller._max_value = this.cont_size;
    ...
    p.create = function (index) {
        ...
        this.controller.options.snapsize = this[this.__dimension] + this.spacing;
        if (!this.loop)
            this.controller._max_value = (this.slidesCount - 1) * (this[this.__dimension] + this.spacing);
...
(function ($) {
    ...
    p.create = function () {
        ...
        if (this.options.wheel) {
            ...
            if ($.browser.mozilla)
                this.$element[0].addEventListener('DOMMouseScroll', this.wheellistener);
            else
    ...
    p.update = function () {
        ...
        if (this.cindex === nindex)
            return;

        if (this.cindex != null)
            this.unselect(this.thumbs[this.cindex]);
        this.cindex = nindex;
        this.select(this.thumbs[this.cindex]);
    ...
    p.changeSlide = function (thumb) {
        if (!this.click_enable || this.cindex === thumb[0].index)
            return;
    ...
    p.destroy = function () {
        if (this.options.wheel) {
            if ($.browser.mozilla)
                this.$element[0].removeEventListener('DOMMouseScroll', this.wheellistener);
            else
...
(function ($) {
    ...
    p.update = function () {
        ...
        if (this.cindex != null)
            this.unselect(this.bullets[this.cindex]);
        this.cindex = nindex;
        this.select(this.bullets[this.cindex]);
    };
