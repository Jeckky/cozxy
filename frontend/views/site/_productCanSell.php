    ...
    <div class="col-lg-3 col-md-4 col-sm-6">
        <div class="tile"> 
            ...
            <div class="footer" id="products-popular-footer" style="max-height: 320px;min-height: 80px;">
                <a href="<?php echo Yii::$app->homeUrl; ?>products/<?= $model->encodeParams(['productId' => $model->productSuppId, 'productSupplierId' => $model->productSuppId]) ?>"><?= $model->title; ?></a>
                <span>
                    <small>in <?php echo $model->category->title; ?></small>
                </span>
                <a href="<?php echo Yii::$app->homeUrl; ?>products/<?= $model->encodeParams(['productId' => $model->productSuppId, 'productSupplierId' => $model->productSuppId]) ?>"><button class="btn btn-primary btn-sm">view</button></a>
            </div>
        </div>
    </div>
<?php } else { ?>
