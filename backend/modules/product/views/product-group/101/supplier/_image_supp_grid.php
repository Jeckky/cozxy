...
<?php
$images = ProductImageSuppliers::productImages($id);
?>
    ...
    if (isset($images) && count($images) > 0 && $images != 0) {
        ...
        foreach ($images as $image):
            ...
            <tr>
                ...
                <td style="vertical-align: middle;">
                    <a href="javascript:orderingImage(<?= $image->productImageId ?>,<?= $total ?>,0)" class="btn btn-warning" style="font-size: 14pt;">-</a>&nbsp;&nbsp;&nbsp;
                    <span id="ordering<?= $image->productImageId ?>" style="font-size: 14pt;"> <?= $image->ordering ?> </span>
                    &nbsp;&nbsp;&nbsp;
                    <a href="javascript:orderingImage(<?= $image->productImageId ?>,<?= $total ?>,1)" class="btn btn-success"style="font-size: 14pt;">+</a>
                </td>
                <td style="vertical-align: middle;">
                    <input type="checkbox" id="imgStatus<?= $image->productImageId ?>" <?= $image->status == 1 ? 'checked' : '' ?> onclick="javascript:changeImageStatus(<?= $image->productImageId ?>)">&nbsp;&nbsp;&nbsp;ใช้งาน
                </td>
                <td style="vertical-align: middle;">
                    <a href="delete-product-supp-image?id=<?= $image->productImageId ?>&&step=<?= $_GET["step"] ?>&&productGroupTemplateId=<?= $_GET["productGroupTemplateId"] ?>&&productGroupId=<?= $_GET["productGroupId"] ?>&&action=delete&&productSuppId=<?= $id ?>" class="btn btn-danger" id="deleteImg<?= $image->productImageId ?>" onclick="return confirm('Are you sure you want to delete this item?');">
