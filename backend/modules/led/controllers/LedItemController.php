...
//        endforeach;
    ...
    public function actionChange($id, $type)
    {
        ...
        if (isset($model) && !empty($model)) {
            if ($type == 'on') {
                file_get_contents('http://' . $model->led->ip . "?id=$model->sortOrder&status=1&r=" . $model->ledColor->r . "&g=" . $model->ledColor->g . "&b=" . $model->ledColor->b . "", NULL, NULL, 0, 0);
                $model->status = 1;
            } else {
                file_get_contents('http://' . $model->led->ip . "?id=$model->sortOrder&status=0&r=" . $model->ledColor->r . "&g=" . $model->ledColor->g . "&b=" . $model->ledColor->b . "", NULL, NULL, 0, 0);
                $model->status = 0;
            }
            $model->save(false);
        }
