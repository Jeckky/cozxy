                                                    ...
                                                    ?>">
                                                        ...
                                                        if ($colIndex == 2 && $rowIndex == 1) { // if ($colIndex == 1 && $rowIndex == 1):
                                                            if ($PickingPoints['type'] == 'booth') {
                                                                ...
                                                                ?>
                                                            <p style="font-size: 20px;font-weight: bold; padding: 20px; background-color: <?= $PickingPoints['color_lid']; ?>;"><?= "Controller" ?></p>
                                                        <?php } ?>
                                                    <?php } else { ?><h4>
                                                            <?php
                                                            if ($row->status == 0) {
                                                                ...
                                                                if ($orderId == $order->orderId) {
                                                                    ?>
                                                                    <a class="btn btn-lg   btn-info"style="margin-top: 10px;" href="<?= Yii::$app->homeUrl; ?>lockers/lockers/save-change-slot?pickingItemsId=<?= $row->pickingItemsId; ?>&orderId=<?= $order->orderId ?>&boxcode=<?= $row->pickingId; ?>&bagNo=<?= $bagno ?>&old=<?= $oldPickingItemsId ?>&code=<?= $code ?>">เลือกช่อง : <?= $row->name; ?></a>
                                                                    <?php
                                                                }
                                                            } else {
                                                                ?>
                                                                <a class="btn btn-lg   btn-info" href="<?php echo Yii::$app->homeUrl; ?>lockers/lockers/save-change-slot?pickingItemsId=<?= $row->pickingItemsId; ?>&orderId=<?= $order->orderId ?>&boxcode=<?= $row->pickingId; ?>&bagNo=<?= $bagno ?>&old=<?= $oldPickingItemsId ?>&code=<?= $code ?>&boxcode=<?= $boxcode ?>">เลือกช่อง : <?= $row->name; ?></a>
                                                                <?php
                                                                //}
                                                            }
                                                            ?>
