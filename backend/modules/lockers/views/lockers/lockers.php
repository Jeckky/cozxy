                                                ...
                                                <td style="border:2px solid ;text-align:center;vertical-align: middle; color: <?php echo $PickingPoints['front']; ?> ;height: <?= $row->height ?>px
                                                        ...
                                                        if ($colIndex == 2 && $rowIndex == 1): // if ($colIndex == 1 && $rowIndex == 1):
                                                            ...
                                                            <h4>
                                                                ...
                                                                if ($row->status == 0) {
                                                                    ...
                                                                    ?>
                                                                    <a class="btn btn-lg   btn-info" href="<?php echo Yii::$app->homeUrl; ?>lockers/lockers/scan-bag?pickingItemsId=<?php echo $row->pickingItemsId; ?>&code=<?php echo $row->code ?>&boxcode=<?php echo $row->pickingId; ?>&model=1&isOpen=1&orderId="<?= OrderItemPacking::findOrderFromPickingItemId($row->pickingItemsId) ?>>เปิดช่อง : <?= $row->name; ?></a>
                                                                    <?php
                                                                } else {
                                                                    if ($Inspector['status'] == 10) {
                                                                        ...
                                                                        ?>
                                                                        <a class="btn btn-lg   btn-info" href="<?php echo Yii::$app->homeUrl; ?>lockers/lockers/scan-bag?pickingItemsId=<?php echo $row->pickingItemsId; ?>&code=<?php echo $row->code ?>&boxcode=<?php echo $row->pickingId; ?>&isOpen=1&model=1&orderId=">เปิดช่อง : <?= $row->name; ?></a>
                                                                        <?php
                                                    ...
                                                    <?php else: ?><h4>
                                                            ...
                                                            if ($row->status == 0) {
                                                                ...
                                                                <a class="btn btn-lg   btn-info" href="<?php echo Yii::$app->homeUrl; ?>lockers/lockers/scan-bag?pickingItemsId=<?php echo $row->pickingItemsId; ?>&code=<?php echo $row->code ?>&boxcode=<?php echo $row->pickingId; ?>&model=1&isOpen=1&orderId="<?= OrderItemPacking::findOrderFromPickingItemId($row->pickingItemsId) ?> style="margin-top: 10px;">เปิดช่อง : <?= $row->name; ?></a>
                                                                <?php
                                                            } else {
                                                                if ($Inspector['status'] == 10) {
                                                                    echo '<span class="label label-danger">ช่อง' . $row->name . ' : มีปัญหา <br>รายละอียด : ' . $Inspector['remark'] . '</span>';
                                                                    ...
                                                                    ?>
                                                                    <a class="btn btn-lg   btn-info" href="<?php echo Yii::$app->homeUrl; ?>lockers/lockers/scan-bag?pickingItemsId=<?php echo $row->pickingItemsId; ?>&code=<?php echo $row->code ?>&boxcode=<?php echo $row->pickingId; ?>&model=1&isOpen=1&orderId=">เปิดช่อง : <?= $row->name; ?></a>
                                                                    <?php
                                                                } else if ($Inspector['status'] == 8) {
                                                                    echo '<span class="label label-warning">ช่อง' . $row->name . ' :รอตรวจสอบจากเจ้าหน้าที่</span>';
                                                                    // echo '<br>';
